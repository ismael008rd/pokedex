...
const getPokemonType = async (pokApiresult) => {
  ...
  return porkemons.map((fulfilled) =>
    fulfilled.types.map((info) => info.type.name)
  );
};

const getPokemonId = (pokApiresult) =>
  pokApiresult.map(({ url }) => {
...
const pegarUrl = async (urimg) => {
  ...
  urimg.forEach((itemsurl) => {
    ...
    const nameUpercase =
      itemsurl.name.charAt(0).toUpperCase() + itemsurl.name.substring(1);

    let Coloritempoke = getoColor(itemsurl).then((color) => {
      color; // acessando o valor resolvido da promessa
    });

    getoColor(itemsurl).then((color) => {
      const colorcard = color;
      li.style.border = ` 5px solid ${getTypeColor(colorcard)}`;
    });
    ...
    img.setAttribute(
      "src",
      `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/back/${resulturl}.png`
    );
...
async function getoColor(item) {
  let respo = await fetch(item.url);
  let usedata = await respo.json();
  const colorColor = usedata.types[0].type.name;
  return colorColor;
}
